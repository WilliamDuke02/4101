...
class App(QWidget):
    ...
    def initUI(self):
        ...
        for i, (name, title) in enumerate(ordered_columns):
            ...
            self.comboboxes[name].setSizePolicy(QSizePolicy.MinimumExpanding, QSizePolicy.Fixed)

            # Place the label and the combo box in the same column i but different rows
            self.grid_layout.addWidget(label, i*2, 0)  # Label in the first row of column i
            self.grid_layout.addWidget(self.comboboxes[name], i*2 + 1, 0)  # Combo box just below the label in column i
        ...
        # Span the export button across all columns, placing it below the last combo box
        self.grid_layout.addWidget(self.export_button, len(ordered_columns)*2, 0, 1, 1, Qt.AlignCenter) 
